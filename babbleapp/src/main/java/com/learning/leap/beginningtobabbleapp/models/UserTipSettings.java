...
public class UserTipSettings {
    ...
    public void loadTurnOnTips(){
       tipsTurnOff =  Utility.readBoolSharedPreferences(Constant.TURN_OFF_TIPS,context);
    }

    public void saveTurnOnTips(){
        if (tipsTurnOff){
            TipReminder.cancelRepeatingIntentService(context);
        }else {
            TipReminder.cancelRepeatingIntentService(context);
            TipReminder.setUpRepeatingIntentService(context);
        }
        Utility.writeBoolenSharedPreferences(Constant.TURN_OFF_TIPS,tipsTurnOff,context);
    }
    ...
    public Boolean getTipsTurnOff() {
        return tipsTurnOff;
    }

    public void setTipsTurnOff(Boolean tipsTurnOff) {
        this.tipsTurnOff = tipsTurnOff;
    }
}
