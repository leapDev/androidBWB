package com.example.ryangunn.beginningtobabble.helper;
...
public class ScheduleBucket {
    ...
    public void findAverageAnswerTime(){
        ...
        if (mAnswerNotificatoinsResults.size() == 0){
            TipReminder.setReminder(startTimeHourForBucket,startTimeMinForBucket,1,mBucketNumber,mContext);
        }else {
    ...
    public void gettime(long totalTimeDifference, int numberOfAnswerNotification){
        ...
        if (notificaionHour > endTimeHourForBucket && notificationMins > endTimeMinForBucket){
            TipReminder.setReminder(endTimeHourForBucket,endTimeMinForBucket,1,mBucketNumber,mContext);
        }else {

            TipReminder.setReminder(notificaionHour, setNoficationMins(notificationMins),1, mBucketNumber, mContext);
        }
    }
    ...
    // divide that into three bucket
    // send a notification during each bucket
    // save the time the user answer the notification
    // schedule the next notification for that bucket at that time
